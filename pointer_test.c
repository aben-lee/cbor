 ...
 * json_pointer_replace('[1,[2,3],4]', '/1/-', 99) → '[1,[2,99],4]'
...
// void cbor_loads_test(const char *input, const char *output) {
//     size_t length;
//     cbor_value_t *src = cbor_json_loads(input, -1);
//     const char *content = cbor_dumps(src, -1);
//     cbor_value_t *val = cbor_loads(content, &length);
//     if (strcmp(src, val)) {
//         fprintf(stderr, "FAIL: cbor_patch('%s', '%s') -> '%s'\n",input, src, val);
//     } else {
//         fprintf(stdout, "PASS: cbor_patch('%s', '%s') -> '%s'\n",input, src, val);
...
// void cbor_string_test(const char *input, int len) {

//   cbor_value_t *result = cbor_init_string("asdfasdfasdfgaw", -1);
...
int main(int argc, char **argv) {
    ...
    replace_test(JSON([1,[2,3],4]), "/1/0", JSON(99), JSON([1, [99, 3], 4]));
    replace_test(JSON([1,[2,3],4]), "/1/-", JSON(99), JSON([1, [2, 99], 4]));
    ...
    // printf("----------test string init---------------------------\n");
    // char *str = "afdssdfasdfas";
