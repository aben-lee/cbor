...
{
    if (ca && cb && ca->type == cb->type && (ca->type == CBOR_TYPE_ARRAY || ca->type == CBOR_TYPE_MAP))
    {
        cbor_value_t *var;
...
{
    if (dst && src && dst->type == src->type && (dst->type == CBOR_TYPE_ARRAY || dst->type == CBOR_TYPE_MAP))
    {
...
cbor_value_t *cbor_loads(const char *src, size_t *length)
{
    ...
    type = (unsigned char)((src[offset] >> 5) & 0x07);  // fixed bug
    ...
    case CBOR_TYPE_UINT:
    {
        ...
        else if (addition == 24 && offset + 1 <= *length)
        { /* uint8_t */
            val->uint = (unsigned char)src[offset];
            offset++;
        }
        else if (addition == 25 && offset + 2 <= *length)
        { /* uint16_t */
            val->uint = be16toh(*(uint16_t *)&src[offset]);
            offset += 2;
        }
        else if (addition == 26 && offset + 4 <= *length)
        { /* uint32_t */
            val->uint = be32toh(*(uint32_t *)&src[offset]);
            offset += 4;
        }
        else if (addition == 27 && offset + 8 <= *length)
        { /* uint64_t */
...
int cbor__dumps(const cbor_value_t *src, cbor_value_t *dst)
{
    ...
    case CBOR_TYPE_SIMPLE:
    {
        ...
        else if (src->simple.ctrl == CBOR_SIMPLE_REAL)
        {
            ...
            {
                exponent -= 1023;
                if (exponent >= -14 && exponent <= 15 && frac_bitcnt <= 10)
                {
                    type |= 25;
                    cbor_blob_append_byte(dst, type);
                ...
                else if (exponent >= -126 && exponent <= 127 && frac_bitcnt <= 23)
                {
                    type |= 26;
                    cbor_blob_append_byte(dst, type);
...
const char *cbor_string(const cbor_value_t *val)
{
    ...
    {
        // size_t lenght;
        // unsigned char * str = base64simple_decode(val->blob.ptr, val->blob.length, &lenght );
        // return str;
        return val->blob.ptr;
    }
    return 0;
...
cbor_value_t *cbor_init_string(const char *str, int len)
{
    ...
    // char* str1 = base64simple_encode((unsigned char),len);
    // len1 = strlen(str1);
    // cbor_blob_append(val, str1, len1);
...
char **string_split(const char *str, const char *f)
{
    ...
    if (offset <= l)
    {
        ...
        strncpy(result[len++], str + offset, l - offset);
    }
    result[len] = NULL;
    return result;
}
